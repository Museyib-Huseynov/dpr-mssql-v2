  ...
  outer: for (const file of files) {
    ...
    if (extension === '.xls' || extension === '.xlsx') {
      ...
      inner: for (let i = 14; i < rows.length; i++) {
        ...
        const gaslift_gas_day = missing_data.includes(row[36]) ? null : row[36];
        const gaslift_system_pressure = missing_data.includes(row[37])
          ? null
          : row[37];
        const pump_depth = missing_data.includes(row[38]) ? null : row[38];
        const pump_frequency = missing_data.includes(row[39]) ? null : row[39];
        const pump_hydrostatic_pressure = missing_data.includes(row[40])
          ? null
          : row[40];
        const esp_pump_size = missing_data.includes(row[41]) ? null : row[41];
        const esp_pump_stages = missing_data.includes(row[42]) ? null : row[42];
        const esp_pump_rate = missing_data.includes(row[43]) ? null : row[43];
        const esp_pump_head = missing_data.includes(row[44]) ? null : row[44];
        ...
        const srp_shaft_diameter = missing_data.includes(row[52])
          ? null
          : row[52];
        const pcp_pump_rate = missing_data.includes(row[53]) ? null : row[53];
